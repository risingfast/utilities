...
const uri01 = "http://gjarman2020.com/cgi-bin/mcaListPeople.cgi";
var iListDisplayLength = 30;                                                   // number of entries displayed in the list
...
function fShowOrHideList() {
    ...
    if (!document.getElementById('added-div')) {                                    // check if added-div does not exist
        ...
        for (let i = 0; i < iListDisplayLength; i++) {
            sHtmlString += `<input id="inputOffset${i}" size="80" value=""><button id="inputBttn${i}" onclick="fGetBttnID(this)">Fetch</button><br>`;
        }
...
async function fFetchPeople() {
    ...
    let response = await fetch(uri01);
    iListDisplayCount = 0;
    if (response.ok) {
