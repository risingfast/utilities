...
int main(void) {
    ...
    char   caSQL[SQL_LEN] = {'\0'};                                // array to hold SQL query string
...
//  setenv("QUERY_STRING", "Username=gjarman&Password=Gssp4atta", 1);
...
// Initialize a connection and connect to the database$$
    ...
    sprintf(caSQL, "SELECT WU.`User Password` as 'Password' "
...
// fetch the password for the user from the database
    ...
    if((row = mysql_fetch_row(res)) != NULL)
    {
        ...
        else if (strcmp(row[0], caPassword) == 0) {
